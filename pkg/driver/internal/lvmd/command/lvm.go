...
func CreateVolumeGroup(ctx context.Context, name string, devices []string) (*VolumeGroup, error) {
	vgCreateArgs := []string{
		"vgcreate",
		"-f",
		"-y",
	...
	for _, device := range devices {
		vgCreateArgs = append(vgCreateArgs, device)
	}

	if err := callLVM(ctx, vgCreateArgs...); err != nil {
		return nil, err
	}
